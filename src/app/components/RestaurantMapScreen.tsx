...
export function RestaurantMapScreen({
  ...
  const [detail, setDetail] = useState<Restaurant | null>(
  ...
  return (
    <div className="relative h-dvh overflow-hidden bg-[#e9eee5] text-soy-ink">
      ...
      <section className="absolute inset-x-0 bottom-0 z-30 px-3 pb-[max(14px,env(safe-area-inset-bottom))]">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex items-center gap-1.5 text-[11px] font-semibold text-brand-primary">
              <span>{t('1인 예상', 'Est. per person', 'تقدير للفرد')} {menuPrice(String(selected.price), language)?.original}</span>
              <span aria-hidden="true">·</span>
              <Users className="size-3" />
              <span>{selected.feedback.reduce((total, item) => total + item.total, 0)} {t('식단 경험', 'dietary experiences', 'تجربة غذائية')}</span>
            </span>
          </span>
