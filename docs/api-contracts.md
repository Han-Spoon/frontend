...
2026-09-15 UI 확장: 메뉴 인식/조리법/프로필/교차접촉별 `checks`와 관찰/추정/미확인 구분을 제안한다. [분식 시연과 상세 JSON 계약](scan-result-filming.md)을 참고한다. 현재 서버 `message`는 가격 아래 음식 설명이 아니라 판정 이유로 보존한다. 서버가 보내지 않은 확률·관점별 근거는 생성하지 않는다.
...
응답 후보를 지도 핀과 기존 선택 가게 카드 UI에 표시한다. 홈에서 `/map`으로 이동하면 전달된 지역 중심 좌표를
우선 사용해 위치 권한을 요청하지 않는다. 직접 `/map`에 진입하거나 사용자가 ‘내 위치’를 누른 경우와 식당 선택창은
브라우저 Geolocation API로 현재 위치를 조회한다. `/map`은 사용자가 지도를 옮긴 뒤 지도 중심 좌표로 후보를 다시 조회한다.
좌표는 컴포넌트 메모리에만 두고 스캔·방문 기록에는 저장하지 않으며 각 화면은 최대 20개 후보를 사용한다.

홈의 조건별 랭킹은 삭제했다. 식당 추천은 동일한 후보 응답의 첫 6곳을 재사용하며 추가 API를 호출하지 않는다.
상호·업종·주소는 서버 값이며 추천 점수·후기·안전 비율은 만들지 않는다. 사진 필드는 아직 없으므로
업종에 대응하는 실사 참고 사진을 사용하고 화면에 명시한다. 상세 정책과 향후 사진 계약은
`restaurant-recommendations.md`에 기록한다. 데모 모드에서만 가상 식당을 사용한다.
